...
2. **行李** — 可勾選的行李清單
3. **計算** — 韓元計算機，即時顯示台幣
4. **記帳** — 分日／分類記帳，含小算盤。每列左滑展開「編輯／刪除」，編輯會把該筆讀回上方表單，按鈕變「更新」
...
- 同步網址寫死在 `SYNC_URL` 常數（`<script>` 開頭附近）。**換過 Apps Script 部署就要回來改這一行。** 填了之後同行的人零設定；留空則退回讓使用者自己在 App 裡貼網址
- 每列帶一個「代號」（`syncUser`）。push 只刪掉並重寫自己代號的列，不同人的帳不會互相蓋掉。`SYNC_URL` 有填時第一次開 App 會自動配一組六位代號，顯示在 CLOUD SYNC 區塊給使用者抄
- 換代號走 `sySwitch()`：**絕對不能直接 push**，那組代號在雲端可能已經有一整趟的紀錄。一律先 fetch 再決定方向，兩邊都有資料時預設保住雲端那份
